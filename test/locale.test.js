// locale.test.js — gt()/setLocale wiring (assignment 012, §3.7) and the "forced-en
// full session shows zero Dutch" bar for the home -> onboarding -> gameplay flow.
// Draait met: npm test
import { test } from 'node:test';
import assert from 'node:assert/strict';

import { gt, setLocale, getLocale } from '../src/game/strings.js';
...
// home -> onboarding -> gameplay flow (the assignment's forced-en bar). Sub-screens
// reached via the home menu (dashboard, records, friends, parent-email, login) are
// assignment 013's full key-set-parity job, not this one.
...
test('gt() never falls back to the Dutch string when a key is missing from en', () => {
  setLocale('en');
  // a key that only exists in the nl map (assignment 013's scope, e.g. dashboard copy)
  assert.equal(gt('dash.title'), 'dash.title'); // raw key, NOT the Dutch '📊 Voor ouders'
  setLocale('nl');
});
