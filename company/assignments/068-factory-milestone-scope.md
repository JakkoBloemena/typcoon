...
- [ ] Milestone scope doc in research/: surfaces, migration plan, cut line, build
      order, per-assignment drafts with acceptance criteria (ids TBD).
- [ ] Save-compatibility stated: existing kids' factories/progress survive
      unchanged (guardrail: never lose a child's earned state).
- [ ] The goals system is specified concretely (what goals, how chosen, how
      surfaced) — not left as a vibe.
- [ ] Playtest-critique assignment drafted as the milestone gate.
