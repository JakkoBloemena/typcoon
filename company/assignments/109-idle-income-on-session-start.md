...
## Verification (tester v109, tick #40)

**Verdict: PASS. Status flipped to `done`.**

Confirmed the fix is present and matches the developer's delivery notes exactly:
`git show 41dac1a` (`src/game/GameScreen.jsx`) contains only the one-line change
`const lastKeyRef = useRef(0)` → `const lastKeyRef = useRef(-Infinity)` plus the
explanatory comment. No other files touched by that commit.

**Setup:** fresh worktree needed `npm install` (node_modules missing) and
`npm install playwright-core --no-save` (test-only, confirmed `package.json` /
`package-lock.json` untouched via `git status`). Built with `npx vite build`, served
with `npx vite preview --port 4293` (my reserved port). `public/**` regenerated by
`npm test`'s `gen-content.mjs` step was reverted with `git checkout -- public/`
before finishing — confirmed clean via `git status`.

**`npm test`: 266/266 green.** `gen-content.mjs` clean (22 URLs + sitemap), `vite
build` clean (101 modules, no warnings), `check-no-dutch-en.mjs` PASS.

**Independent live probe** (`qa-scripts/109-tester-verify.mjs`, written from
scratch — different save shape/magnitude/naming than the developer's
`109-dev-verify.mjs` and the original `103-tester-idle*.mjs` scripts, reusing only
the general save-injection technique): a save with `buildings: {typewriter:40,
printer:18, robotarm:6, assembly:30}`, `rebirths:3`, `lastDay` set to 10 days ago
(to also exercise the daily "welcome back" streak overlay path, since the original
103 repro flagged that as a thing to rule out). Five scenarios, all against the
real fixed build in a real Chromium instance:

1. **Fresh entry, zero keystrokes, idle 6+ seconds** — coins flat at `5000` across
   8 samples (t=761ms .. t=6089ms). No mint.
2. **Leave the play view (nav to Factory) and re-enter, still zero keystrokes ever**
   — coins flat at `5000` across 6 samples after re-entry (t=702ms .. t=4249ms). This
   exercises a second effect-mount within the same page load, which the `0` seed
   would also have leaked on (a fresh `performance.now() - 0` window at any mount
   within `ACTIVE_WINDOW_MS` of *navigation* start, not component-mount start — so
   this check has real discriminating power, not just a repeat of scenario 1). Had
   to dismiss a "Welkom terug!" daily-streak overlay (`daily.js`, unrelated by-design
   feature — multiplies future exercise rewards, does not touch `tycoon.coins`
   itself, confirmed by scenario 1 already passing while it was on screen) to reach
   the nav button, same as a real player would.
3. **Full page reload while sitting in the play view, then re-enter, zero keystrokes
   ever** — coins flat at `5000` across 8 samples (t=755ms .. t=6089ms). This is the
   scenario the assignment specifically calls out ("reload page") and gives the
   fresh `performance.now()` origin the most favorable conditions to leak under the
   old `0` seed.
4. **Real faucet intact** — 6 real `page.keyboard.press()` keystrokes (window-level
   keydown, same path `TypingSurface.jsx` → `handleKeystroke` uses) over ~2.7s:
   coins grew `5000` → `259149.34` (representative run; reran the full probe twice,
   both times all 5 scenarios passed, magnitudes varied slightly with real elapsed
   time between ticks as expected).
5. **Decay intact** — after the last keystroke, production kept ticking for a bit
   (still within `ACTIVE_WINDOW_MS`) then flattened: samples `514010.63,
   514010.63, 514010.63` for the last three of seven post-typing samples
   (t=3225ms..5654ms), confirming the 3.5s activity window still gates production
   off correctly and the fix did not overcorrect into "never produces."

Ran the whole probe twice end-to-end (server restarted between runs) — identical
pass/fail shape both times, only the exact coin magnitudes varied with real timing
jitter, as expected.

**Scope note on my own testing:** attempted to also verify discriminating power by
temporarily reverting the one-line fix and re-running the probe (the way the
developer did in their own delivery notes) — this was correctly blocked by the
harness's write-surface guardrail (`src/**` is out of scope for this lane). Skipped
that step rather than working around it; the developer's own before/after
comparison (documented in their delivery notes, `qa-scripts/103-tester-idle.mjs`
and `103-tester-idle2.mjs` run unmodified pre/post-fix) already establishes
discriminating power, and my 5 independently-constructed scenarios against the
fixed build all pass cleanly with no ambiguity.

**Per-criterion verdict:**
1. `lastKeyRef` seeded so `active` is false until a real keystroke — **met**,
   confirmed by reading the committed diff (`git show 41dac1a`).
2. 6+ second idle repro shows `tycoon.coins` unchanged — **met**, and extended:
   also unchanged across leave/re-enter and full page reload, not just the
   original single-entry repro.
3. No regression to legitimate active-typing production — **met**: typing produces
   real growth, and decay after the last keystroke still works.
4. `npm test` stays green — **met**: 266/266.

**No new unrelated defect found** during this verification pass. Reserved id 118
was not consumed.

**Files touched by this lane:** `company/assignments/109-idle-income-on-session-
start.md` (status → `done`, this section), `qa-scripts/109-tester-verify.mjs` (new,
independent probe, kept as a permanent regression check alongside the developer's
own `qa-scripts/109-dev-verify.mjs`). No `src/**` changes.
